    ...
    private void reconcileLegacy(Transaction tx) {
        ...
        try {
            ...
            tx.setTx_update_trace(
                    "AIRTEL_STATUS_POLL: providerStatus="
                            + providerStatus
                            + ";httpStatus="
            ...
            if (!"success".equalsIgnoreCase(result)) {
                logger.warning(
                        "Canonical Airtel transaction finalization did not complete for "
                                + tx.getTx_unique_id()
                                + ": "
                                + result);
            }
            ...
            logger.log(
                    Level.WARNING,
                    "Airtel legacy status reconciliation failed for " + tx.getTx_unique_id(),
                    e);
        }
    }
