...
public class DoPayGateway {
    ...
    private static void configureAirtelOpenApiEndpoints(
            ...
            boolean useMerchantCreds,
            NamedParameterJdbcTemplate jdbcTemplate) {
        Setting state = Common.getSettings("application_settings_state", jdbcTemplate);
        gateway.setTransactionContext(
                state == null || "production".equalsIgnoreCase(state.getSetting_value())
                        ? "PRODUCTION"
                        : "SANDBOX",
                "UG",
                "UGX");
    ...
    private static java.math.BigDecimal calculateLegacyFee(
            java.math.BigDecimal amount, Double fee, String method) {
        java.math.BigDecimal rate = java.math.BigDecimal.valueOf(fee);
        java.math.BigDecimal value;
        if (rate.signum() < 0)
            throw new net.citotech.cito.gateway.PaymentGatewayException("Fee cannot be negative");
        if ("percentage".equalsIgnoreCase(method) || "PERCENTAGE".equalsIgnoreCase(method)) {
            if (rate.compareTo(java.math.BigDecimal.valueOf(100)) > 0)
                throw new net.citotech.cito.gateway.PaymentGatewayException(
                        "Percentage fee exceeds 100 percent");
            value = amount.multiply(rate).movePointLeft(2);
        } else if ("flat".equalsIgnoreCase(method) || "FLAT_FEE".equalsIgnoreCase(method)) {
            value = rate;
        } else {
            throw new net.citotech.cito.gateway.PaymentGatewayException(
                    "Unsupported fee calculation method");
        }
        return net.citotech.cito.money.MoneyAmount.of(value).asBigDecimal();
    }

    public static Double getCustomerInboundCharges(Double amount, GatewayChargeDetails charges) {
        return getCustomerInboundChargesDecimal(java.math.BigDecimal.valueOf(amount), charges)
                .doubleValue();
    }

    public static java.math.BigDecimal getCustomerInboundChargesDecimal(
            java.math.BigDecimal amount, GatewayChargeDetails charges) {
        return calculateLegacyFee(
                amount,
                charges.getCustomerInboundCharge(),
                charges.getCustomerInboundChargeMethod());
    }

    public static Double getCustomerOutboundCharges(Double amount, GatewayChargeDetails charges) {
        return getCustomerOutboundChargesDecimal(java.math.BigDecimal.valueOf(amount), charges)
                .doubleValue();
    }

    public static java.math.BigDecimal getCustomerOutboundChargesDecimal(
            java.math.BigDecimal amount, GatewayChargeDetails charges) {
        return calculateLegacyFee(
                amount,
                charges.getCustomerOutboundCharge(),
                charges.getCustomerOutboundChargeMethod());
    }

    public static Double getCostOfInboundCharges(Double amount, GatewayChargeDetails charges) {
        return getCostOfInboundChargesDecimal(java.math.BigDecimal.valueOf(amount), charges)
                .doubleValue();
    }

    public static java.math.BigDecimal getCostOfInboundChargesDecimal(
            java.math.BigDecimal amount, GatewayChargeDetails charges) {
        return calculateLegacyFee(
                amount, charges.getCostOfInboundPayment(), charges.getCostOfPayInMethod());
    }

    public static Double getCostOfOutboundCharges(Double amount, GatewayChargeDetails charges) {
        return getCostOfOutboundChargesDecimal(java.math.BigDecimal.valueOf(amount), charges)
                .doubleValue();
    }

    public static java.math.BigDecimal getCostOfOutboundChargesDecimal(
            java.math.BigDecimal amount, GatewayChargeDetails charges) {
        return calculateLegacyFee(
                amount, charges.getCostOfOutboundPayment(), charges.getCostOfPayOutMethod());
    }

    public static Double getCustomertOfOutboundCharges(
            Double amount, GatewayChargeDetails chargeDetails) {
        return getCostOfOutboundCharges(amount, chargeDetails);
    }
