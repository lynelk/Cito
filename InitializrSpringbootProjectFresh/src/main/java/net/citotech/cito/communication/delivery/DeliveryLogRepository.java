...
public class DeliveryLogRepository {
    ...
    public int updateStatus(long id, DeliveryStatus status, String trace, String gwResponse) {
        return jdbcTemplate.update(
                ...
                new MapSqlParameterSource()
                        ...
                        .addValue("gw_response", gwResponse));
    }
    ...
    public int updateProviderMessageId(long id, String providerMessageId) {
        ...
        return jdbcTemplate.update(
                "UPDATE communication_message_deliveries SET provider_message_id=:provider_message_id WHERE id=:id",
                new MapSqlParameterSource()
    ...
    public int markBilled(long id) {
        return jdbcTemplate.update(
                "UPDATE communication_message_deliveries SET billed_flag='Y' WHERE id=:id AND billed_flag='N'",
    ...
    public List<MessageDelivery> listForMerchant(long merchantId, int limit) {
        return jdbcTemplate.query(
                "SELECT id, merchant_id, channel, provider_code, reference_type, reference_id, recipient,"
                        + " status, trace, gw_response, charged_amount, billed_flag"
                        + " FROM communication_message_deliveries WHERE merchant_id=:merchant_id ORDER BY id DESC LIMIT :limit",
                new MapSqlParameterSource()
                        .addValue("merchant_id", merchantId)
