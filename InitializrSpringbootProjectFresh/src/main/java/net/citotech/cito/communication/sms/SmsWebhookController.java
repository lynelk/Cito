...
public class SmsWebhookController {
    ...
    public SmsWebhookController(
            SmsConversationService conversationService, ObjectMapper objectMapper) {
    ...
    public ResponseEntity<?> inbound(
        ...
        try {
            ...
            return ResponseEntity.accepted()
                    .body(
                            conversationService.receiveInbound(
                                    token,
                                    ...
                                    first(
                                            ...
                                            "smsMessageId"),
                                    first(
                                            payload, "from", "From", "sender", "phone", "source",
                                            "msisdn"),
                                    first(
                                            payload,
                                            "to",
    ...
    public ResponseEntity<?> delivery(
        ...
        try {
            ...
            return ResponseEntity.accepted()
                    .body(
                            conversationService.receiveDeliveryReceipt(
                                    token,
                                    first(payload, "providerCode", "provider", "gateway"),
                                    first(
                                            payload,
    ...
    private Map<String, Object> payload(HttpServletRequest request) {
        ...
        if (contentType != null && contentType.toLowerCase().contains("json")) {
            try {
                ...
                // Parameter-based provider callbacks remain valid even when an optional body is
                // absent.
            }
        }
        return values;
