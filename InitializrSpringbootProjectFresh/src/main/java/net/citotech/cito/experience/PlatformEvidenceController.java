...
/**
 * Administrator-only evidence scorecard. Every number is computed from durable records already
 * owned by Cito. Missing evidence stays missing; targets are never converted into achieved metrics.
 */
@RestController
...
public class PlatformEvidenceController {
    ...
    private Map<String, Long> commercialEvidence() {
        ...
        values.put(
                ...
                count(
                        "SELECT COUNT(*)",
                        "FROM founding20_merchants",
                        "WHERE programme_status='CANDIDATE'"));
        values.put(
                "founding20Active",
                count(
                        "SELECT COUNT(*)",
                        "FROM founding20_merchants",
                        "WHERE programme_status IN ('ACTIVE','ONBOARDING','LIVE')"));
        values.put(
                "founding20Live",
                count(
                        "SELECT COUNT(*)",
                        "FROM founding20_merchants",
                        "WHERE programme_status='LIVE'"));
        values.put(
                "activePackageAssignments",
                count(
                        "SELECT COUNT(*)",
                        "FROM merchant_commercial_package_assignments",
                        "WHERE status='ACTIVE'",
        ...
        values.put(
                ...
                count(
                        "SELECT COUNT(*)",
                        "FROM embedded_partner_programmes",
                        "WHERE programme_status='LIVE'"));
        return values;
    }
    ...
    private Map<String, Long> developerEvidence() {
        ...
        values.put(
                ...
                count(
                        "SELECT COUNT(*)",
                        "FROM developer_project_environments",
                        "WHERE environment='PRODUCTION'",
                        "AND status='ACTIVE'",
                        "AND production_eligible='YES'"));
        values.put(
                "apiRequests7d",
                count(
                        "SELECT COUNT(*)",
                        "FROM developer_api_request_log",
                        "WHERE created_at>=TIMESTAMPADD(DAY,-7,CURRENT_TIMESTAMP)"));
        values.put(
                "activeApiMerchants30d",
                count(
                        "SELECT COUNT(DISTINCT merchant_id)",
                        "FROM developer_api_request_log",
                        "WHERE created_at>=TIMESTAMPADD(DAY,-30,CURRENT_TIMESTAMP)"));
        values.put(
    ...
    private Map<String, Long> adoptionEvidence() {
        ...
        values.put(
                ...
                count(
                        "SELECT COUNT(*)",
                        "FROM merchant_onboarding_workflows",
                        "WHERE status='LIVE'"));
        values.put(
