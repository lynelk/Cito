 ...
 * Financial finalization stays on the caller and never runs after an abandoned lookup. A
 * non-interruptible provider can occupy at most two daemon workers; excess work fails closed.
 */
@Component
    ...
    <T> T execute(Callable<T> operation, long timeoutMillis) {
        ...
        try {
            ...
            throw new PaymentGatewayException(
                    "Provider verification interrupted; outcome remains pending");
        } catch (ExecutionException error) {
            if (error.getCause() instanceof RuntimeException cause) throw cause;
            throw new PaymentGatewayException(
                    "Provider verification failed; outcome remains pending");
        } catch (java.util.concurrent.TimeoutException error) {
            throw new PaymentGatewayException(
                    "Provider verification deadline exceeded; outcome remains pending");
        } finally {
            future.cancel(true);
        }
    }
