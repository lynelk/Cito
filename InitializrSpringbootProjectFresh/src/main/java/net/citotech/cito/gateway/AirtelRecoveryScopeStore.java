...
public final class AirtelRecoveryScopeStore {
    ...
    public static void record(
        ...
        try {
            jdbc.update(
                    "INSERT INTO airtel_recovery_scopes (merchant_id, transaction_reference, operation, identity_hash)"
                            + " VALUES (:merchant,:reference,:operation,:identity)",
                    parameters);
    ...
    public static void require(
        ...
        List<String> stored =
                jdbc.queryForList(
                        "SELECT identity_hash FROM airtel_recovery_scopes"
                                + " WHERE merchant_id=:merchant AND transaction_reference=:reference AND operation=:operation",
                        parameters(reference, merchantId, operation, identity),
                        String.class);
        if (stored.size() != 1
            ...
            throw new PaymentGatewayException(
                    "Airtel recovery requires the original recorded credential owner; reconcile missing or conflicting provenance");
        }
    }

    static String fingerprint(
            String source, String baseUrl, String clientId, String country, String currency) {
        ...
        if (uri.getHost() == null
            ...
            throw new PaymentGatewayException(
                    "Airtel recovery endpoint must identify one provider origin");
        String endpoint =
    ...
    private static MapSqlParameterSource parameters(
            String reference, Long merchantId, String operation, String identity) {
        if (merchantId == null
                || merchantId <= 0
                || blank(reference)
        ...
        return new MapSqlParameterSource("merchant", merchantId)
