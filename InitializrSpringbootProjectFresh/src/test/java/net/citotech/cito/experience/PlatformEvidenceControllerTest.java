...
class PlatformEvidenceControllerTest {
    ...
    private final PlatformEvidenceController controller = new PlatformEvidenceController(access, jdbc);

    @Test
    void administratorGetsDurableEvidenceWithoutTargetInflation() {
        var admin = new ExperienceAccessContext.Access("admin@example.invalid", null, true, "ADMIN");
        ...
        when(jdbc.queryForList(anyString())).thenReturn(List.of(Map.of(
                "providerCode", "*", "channelCode", "*", "requiredScenarios", 11L, "approvedScenarios", 0L)));
        ...
        assertThat(result.get("providerCertification")).isEqualTo(List.of(Map.of(
                "providerCode", "*", "channelCode", "*", "requiredScenarios", 11L, "approvedScenarios", 0L)));
    }

    @Test
    void merchantCannotReadPlatformCommercialEvidence() {
        var merchant = new ExperienceAccessContext.Access("merchant@example.invalid", 42L, false, "OWNER");
        when(access.require(request, authentication)).thenReturn(merchant);
        doThrow(new ResponseStatusException(org.springframework.http.HttpStatus.FORBIDDEN, "Administrator access required"))
                .when(access).requireAdmin(merchant);

        org.assertj.core.api.Assertions.assertThatThrownBy(() -> controller.scorecard(request, authentication))
