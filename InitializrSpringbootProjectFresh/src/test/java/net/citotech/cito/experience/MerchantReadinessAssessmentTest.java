...
class MerchantReadinessAssessmentTest {
    ...
    private List<Map<String, Object>> complete() {
        return new ArrayList<>(
                List.of(
                        step("SANDBOX_CONFIGURED", "COMPLETED"),
                        step("INTEGRATION_TESTED", "COMPLETED"),
                        step("PROVIDER_CERTIFIED", "COMPLETED"),
                        step("GO_LIVE_APPROVED", "COMPLETED")));
    }

    @Test
    void emptyStepsNeverBecomeReadyThroughVacuousTruth() {
    ...
    @Test
    void liveLabelAloneCannotInventActivationEvidence() {
    ...
    @Test
    void configuredAndTestedAreNotCertifiedOrEnabled() {
        var steps =
                List.of(
                        step("SANDBOX_CONFIGURED", "COMPLETED"),
                        step("INTEGRATION_TESTED", "COMPLETED"));
    ...
    @Test
    void waivedOrUntimestampedTestsDoNotClaimVerification() {
        for (String status : List.of("WAIVED", "WAIVED_LEGACY", "SKIPPED")) {
            var result =
                    MerchantReadinessAssessment.assess(
                            Map.of(), List.of(step("SANDBOX_CONFIGURED", status)));
    ...
    @Test
    void completedEvidenceAllowsReadinessButNotAutomaticActivation() {
        var result =
                MerchantReadinessAssessment.assess(
                        Map.of("status", "GO_LIVE_APPROVED"), complete());
    ...
    @Test
    void completeActivationIsStillDistinctFromIndividualProviderEnablement() {
        var steps = complete();
        steps.add(step("PRODUCTION_ACTIVATED", "COMPLETED"));
        var result =
                MerchantReadinessAssessment.assess(
                        Map.of("status", "LIVE", "activatedAt", "2026-09-11T00:00:00Z"), steps);
