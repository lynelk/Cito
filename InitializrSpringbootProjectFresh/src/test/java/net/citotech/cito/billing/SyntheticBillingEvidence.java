...
public final class SyntheticBillingEvidence {
    ...
    public static void recordCompleteSource(NamedParameterJdbcTemplate jdbc,
            BillingInvoiceRepository invoices, long invoiceId) {
        var invoice = invoices.find(invoiceId).orElseThrow();
        var through = Timestamp.valueOf(invoice.periodEnd().atTime(23, 59, 59));
        jdbc.update("INSERT INTO billing_source_watermarks"
                ...
                new MapSqlParameterSource("tenant", invoice.billingTenantId()).addValue("through", through));
    }
}
