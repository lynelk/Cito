...
class BoundedProviderVerificationTest {
    ...
    void slowProviderCannotHoldTheRecoveryCallerIndefinitely() throws Exception {
        ...
        try {
            assertThatThrownBy(() -> executor.execute(() -> {
                ...
                return "SUCCESSFUL";
            }, 100)).isInstanceOf(PaymentGatewayException.class).hasMessageContaining("deadline");
    ...
    void providerErrorsAreNotConvertedToSuccessOrFailureOutcomes() {
        ...
        try {
            assertThatThrownBy(() -> executor.execute(() -> {
                throw new PaymentGatewayException("synthetic provider unavailable");
            }, 1000)).hasMessage("synthetic provider unavailable");
        } finally {
            executor.close();
        }
    }
