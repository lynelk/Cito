...
1. Complete and verify Airtel durable asynchronous recovery for both merchant-specific and platform credential sources. This repair does **not** implement an Airtel correlation table, automatic status-polling worker or end-to-end callback settlement. Do not present the timeout exclusion as automatic recovery. Reconcile aged pending Airtel items through the authenticated provider status process, preserve the original reference and exact credential scope, and apply terminal outcomes through the existing settlement service with maker-checker/audit controls. Never release a hold or resubmit based solely on elapsed time or a callback payload.
