...
function Group({ title, values }: { title: string; values: MetricGroup }): React.ReactElement {
  ...
  return <article className="pm-card" aria-label={title}>
    ...
    {entries.length ? entries.map(([key, value]) => <div key={key} className="pm-evidence-row"><span>{labels[key] || key}</span><strong>{value.toLocaleString()}</strong></div>) : <p>No durable evidence recorded yet.</p>}
  </article>;
}
...
export default function PlatformEvidencePanel(): React.ReactElement {
  ...
  if (query.error || !query.data) {
    ...
    return <section className="pm-error" role="alert">Commercial and adoption evidence could not be loaded: {message}. No zero or success status is assumed.</section>;
  }
  const data = query.data;
  const providerCertification = Array.isArray(data.providerCertification) ? data.providerCertification : [];
  const basis = data.evidenceBasis || 'EVIDENCE_BASIS_UNKNOWN';
  return <section aria-label="Commercial and adoption evidence">
    ...
    <div className="pm-grid">
      ...
      <article className="pm-card" aria-label="Provider certification evidence">
        ...
        <code>{basis} · targets-as-actuals={String(Boolean(data.targetsReportedAsActuals))}</code>
      </article>
    </div>
